...
**[2026-08-26 — ĐÃ ĐÓNG] Điều tra khả năng đổi hẳn thư viện Chart Engine (khỏi `iztro`) để khớp tuvi.vn 100% — kết luận: KHÔNG đổi, giữ `iztro`.**

Sau khi phát hiện 1 số điểm khác biệt (Thiên Khôi, thang độ sáng, vài tạp tinh), người dùng đặt câu hỏi liệu có nên thay `iztro` bằng thư viện/mã nguồn khác để khớp tuvi.vn tuyệt đối. Đã khảo sát bằng 2 hướng độc lập:

- **Khảo sát bằng agent (web search):** không tìm thấy bằng chứng công khai nào cho thấy tuvi.vn dùng 1 thư viện mã nguồn mở cụ thể. `Renhuai123/ziwei-doushu` (thư viện phổ biến nhất, ~3.7k sao) dùng hệ **Phi Tinh phái** — khác hẳn khái niệm với `iztro`/tuvi.vn (cùng hệ Tam Hợp phái), không tương thích. Các thư viện Việt Nam nhỏ hơn (`doanguyen/lasotuvi`, `hieudo-ursa/ansaotuvi`) không có tài liệu trường phái/nguồn tham khảo rõ ràng để đối chiếu công thức như cách đã làm với `iztro`.
- **Người dùng tự điều tra sâu công thức `doanguyen/lasotuvi`, đối chiếu tay với 1 case thật lấy từ tuvi.vn** (case "Thịnh", Quý Mùi 2003, 23/7 âm lịch, giờ Sửu, Nam) — tính tay theo công thức `lasotuvi` cho ra 14/14 chính tinh khớp dữ liệu tuvi.vn hiển thị.
- **Verify độc lập bằng `iztro` thật (không suy luận qua trung gian):** chạy `buildChart()` cho đúng case Thịnh — kết quả **khớp TUYỆT ĐỐI 14/14 chính tinh** với dữ liệu tuvi.vn (đúng cả vị trí lẫn cách ghép cặp từng cung). Về Tuần/Triệt: `iztro` cho Triệt tại Sửu — xác nhận đây chính là 1 trong 2 giá trị `jieluIndex`(Tý)/`kongwangIndex`(Sửu) mà `algorithm: 'zhongzhou'` gộp lại chỉ chọn 1 (đúng cơ chế đã điều tra ở case Phạm Duy phía trên) — và cặp Tý-Sửu này khớp đúng công thức `timTriet()` của `lasotuvi` cho can Quý. **Không phải 2 công thức khác nhau — cùng 1 công thức gốc, chỉ khác cách `zhongzhou` gộp 1 trong 2 giá trị thay vì giữ cả 2.**

**Kết luận: bằng chứng (14/14 chính tinh khớp tuyệt đối ở CẢ 2 case đã test — Phạm Duy và Thịnh) ủng hộ mạnh việc GIỮ `iztro`, không đổi thư viện.** Các khác biệt còn lại (Thiên Khôi, thang độ sáng 7 vs 5 mức, cách gộp Tuần/Triệt 1 vs 2 giá trị) là khác biệt đã hiểu rõ nguyên nhân qua điều tra code, không phải lỗi hay dấu hiệu cần thay thế Chart Engine. Đổi thư viện sẽ đánh đổi 1 bộ vấn đề đã biết rõ, đã phân loại, có bằng chứng code cụ thể — lấy 1 "hộp đen" mới chưa được điều tra ở mức tương đương. **Đóng thread điều tra thư viện tại đây.**
